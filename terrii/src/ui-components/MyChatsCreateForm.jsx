...
import { createMyChats } from "../graphql/mutations";
const client = generateClient();
export default function MyChatsCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    setUnreadMessages(initialValues.unreadMessages);
    setLastMessageDateTime(initialValues.lastMessageDateTime);
    setLastMessage(initialValues.lastMessage);
    setChatName(initialValues.chatName);
    setChatImageS3URL(initialValues.chatImageS3URL);
    setErrors({});
  };
  const validations = {
    unreadMessages: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: createMyChats.replaceAll("__typename", ""),
            variables: {
              input: {
                ...modelFields,
              },
      ...
      {...getOverrideProps(overrides, "MyChatsCreateForm")}
      {...rest}
    >
      <SwitchField
