...
import { createGuidance } from "../graphql/mutations";
...
export default function GuidanceCreateForm(props) {
  ...
  const resetStateValues = () => {
    ...
    setLongDesc(initialValues.longDesc);
    setThumbURL(initialValues.thumbURL);
    setDate(initialValues.date);
    setTime(initialValues.time);
    setEstTime(initialValues.estTime);
    setDeleted(initialValues.deleted);
    setLocation(initialValues.location);
    setAuthor(initialValues.author);
    setS3ObjectKey(initialValues.S3ObjectKey);
    setStatus(initialValues.status);
    setActive(initialValues.active);
    setLowerCaseTitle(initialValues.lowerCaseTitle);
    setLowerCaseShortDesc(initialValues.lowerCaseShortDesc);
    setScheduleDateTime(initialValues.scheduleDateTime);
    setErrors({});
  };
  const validations = {
    title: [],
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: createGuidance.replaceAll("__typename", ""),
            variables: {
              input: {
                ...modelFields,
              },
