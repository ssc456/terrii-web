...
import { createQuestionGroup } from "../graphql/mutations";
const client = generateClient();
export default function QuestionGroupCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    ...
    setImageS3ObjectKey(initialValues.imageS3ObjectKey);
    setDeleted(initialValues.deleted);
    setType(initialValues.type);
    setStatus(initialValues.status);
    setGroupLevel(initialValues.groupLevel);
    setErrors({});
  };
  const validations = {
    name: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: createQuestionGroup.replaceAll("__typename", ""),
            variables: {
              input: {
                ...modelFields,
              },
      ...
      {...getOverrideProps(overrides, "QuestionGroupCreateForm")}
      {...rest}
    >
      <TextField
        label="Name"
