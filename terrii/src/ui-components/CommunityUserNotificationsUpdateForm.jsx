...
import { getCommunityUserNotifications } from "../graphql/queries";
import { updateCommunityUserNotifications } from "../graphql/mutations";
const client = generateClient();
export default function CommunityUserNotificationsUpdateForm(props) {
  const {
    id: idProp,
    communityUserNotifications: communityUserNotificationsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = communityUserNotificationsRecord
      ? { ...initialValues, ...communityUserNotificationsRecord }
      : initialValues;
  ...
  const [
    communityUserNotificationsRecord,
    setCommunityUserNotificationsRecord,
  ] = React.useState(communityUserNotificationsModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getCommunityUserNotifications.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getCommunityUserNotifications
        : communityUserNotificationsModelProp;
      setCommunityUserNotificationsRecord(record);
    };
    queryData();
  }, [idProp, communityUserNotificationsModelProp]);
  React.useEffect(resetStateValues, [communityUserNotificationsRecord]);
  const validations = {
    NotificationType: [],
    seen: [],
  };
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateCommunityUserNotifications.replaceAll(
              "__typename",
              ""
            ),
            variables: {
              input: {
                id: communityUserNotificationsRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "CommunityUserNotificationsUpdateForm")}
      {...rest}
    >
      <SelectField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || communityUserNotificationsModelProp)}
