...
import { getQuestionAnswers } from "../graphql/queries";
import { updateQuestionAnswers } from "../graphql/mutations";
const client = generateClient();
export default function QuestionAnswersUpdateForm(props) {
  const {
    id: idProp,
    questionAnswers: questionAnswersModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = questionAnswersRecord
      ? { ...initialValues, ...questionAnswersRecord }
      : initialValues;
    setAnswer(cleanValues.answer);
    setHelpText(cleanValues.helpText);
    setNextQuestionID(cleanValues.nextQuestionID);
    setDeleted(cleanValues.deleted);
    setErrors({});
  };
  const [questionAnswersRecord, setQuestionAnswersRecord] = React.useState(
    questionAnswersModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getQuestionAnswers.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getQuestionAnswers
        : questionAnswersModelProp;
      setQuestionAnswersRecord(record);
    };
    queryData();
  }, [idProp, questionAnswersModelProp]);
  React.useEffect(resetStateValues, [questionAnswersRecord]);
  const validations = {
    answer: [],
    helpText: [],
    nextQuestionID: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateQuestionAnswers.replaceAll("__typename", ""),
            variables: {
              input: {
                id: questionAnswersRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "QuestionAnswersUpdateForm")}
      {...rest}
    >
      <TextField
        label="Answer"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || questionAnswersModelProp)}
