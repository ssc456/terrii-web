...
import { createCarePlanOutputs } from "../graphql/mutations";
const client = generateClient();
export default function CarePlanOutputsCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    setCarePlanJSON(initialValues.carePlanJSON);
    setDeleted(initialValues.deleted);
    setCarePlanName(initialValues.carePlanName);
    setErrors({});
  };
  const validations = {
    carePlanJSON: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: createCarePlanOutputs.replaceAll("__typename", ""),
            variables: {
              input: {
                ...modelFields,
              },
      ...
      {...getOverrideProps(overrides, "CarePlanOutputsCreateForm")}
      {...rest}
    >
      <TextField
